package com.crudzilla.http.proxy;
...
public class ProxyServlet extends HttpServlet {
  ...
  private static final String P_SENDURLFRAGMENT = "sendurlfrag";
  /* MISC */

  protected static Map _globalConfig = new HashMap();
  ...
  */
  ...
  public void init(ServletConfig servletConfig) throws ServletException {
    ...
    if (doLogStr != null) {
      ...
      config.put(P_LOG,doLogStr);
    }
    
    String doForwardIPString = servletConfig.getInitParameter(P_FORWARDEDFOR);
    if (doForwardIPString != null) {
    	//this.doForwardIP = Boolean.parseBoolean(doForwardIPString);
      config.put(P_FORWARDEDFOR,doForwardIPString);
    }  


    String sendUrlFragmentString = servletConfig.getInitParameter(P_SENDURLFRAGMENT);
    if (sendUrlFragmentString != null) {
    	//this.doForwardIP = Boolean.parseBoolean(doForwardIPString);
      config.put(P_SENDURLFRAGMENT,sendUrlFragmentString);
    }    
    
    
    String targetUri = servletConfig.getInitParameter(P_TARGET_URI);
    config.put(P_TARGET_URI,targetUri);
    ...
    */
    if(targetUri != null)
    	ProxyServlet.configure("default",config);
  }

  public static void configure(String path,Map config){
      Map newConfig = new HashMap();
    
      String doLogStr = (String)config.get(P_LOG);
      if (doLogStr != null) {
        newConfig.put("doLog",Boolean.parseBoolean(doLogStr));
      }
      else
      {
        newConfig.put("doLog",false);
      }
      
      String doForwardIPString = (String)config.get(P_FORWARDEDFOR);
      if (doForwardIPString != null) {
          //this.doForwardIP = Boolean.parseBoolean(doForwardIPString);
          newConfig.put("doForwardIP",Boolean.parseBoolean(doForwardIPString));
      }
      else
      {
          newConfig.put("doForwardIP",true);
      }
    
    
      String sendUrlFragmentString = (String)config.get(P_SENDURLFRAGMENT);
      if (sendUrlFragmentString != null) {
          newConfig.put("doSendUrlFragment",Boolean.parseBoolean(sendUrlFragmentString));
      }
      else
      {
          newConfig.put("doSendUrlFragment",true);
      }    
  
      URI targetUriObj = null;
      try 
      {
        targetUriObj = new URI((String)config.get(P_TARGET_URI));
        newConfig.put("targetUriObj",targetUriObj);
        
      } catch (Exception e) {
        throw new RuntimeException("Trying to process targetUri init parameter: "+e,e);
      }
      String targetUri = targetUriObj.toString();
      newConfig.put("targetUri",targetUri);
  
      HttpParams hcParams = new BasicHttpParams();
      readConfigParam(hcParams, ClientPNames.HANDLE_REDIRECTS, Boolean.class,config);
      HttpClient proxyClient = createHttpClient(hcParams);      
      newConfig.put("proxyClient",proxyClient);
    
      _globalConfig.put(path,newConfig);
  }
  ...
  protected static HttpClient createHttpClient(HttpParams hcParams) {
    try {
  ...
  protected static void readConfigParam(HttpParams hcParams, String hcParamName, Class type,Map config) {
    String val_str = /*getServletConfig().getInitParameter*/(String)config.get(hcParamName);
    if (val_str == null)
      return;
    Object val_obj;
  ...
  public void destroy() {
    ...
    for (Map.Entry<String, Map> entry : ((Map<String, Map>)_globalConfig).entrySet()) { 
      	String key = entry.getKey();
      	Map value = entry.getValue();
      
      	HttpClient proxyClient = (HttpClient)value.get("proxyClient");
  ...
  protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
    ...
    String targetProxy      = pathParts[1];
    String proxyRequestUri  = null;    
            
        
    //get config for this proxy
    Map currentProxyConfig = (Map)_globalConfig.get(targetProxy); 
        
    if(currentProxyConfig == null)
    	currentProxyConfig = (Map)_globalConfig.get("default");     
        
    String     targetUri      = (String)currentProxyConfig.get("targetUri");    
    URI        targetUriObj   = (URI)currentProxyConfig.get("targetUriObj");     
        
        
    if(targetUri.isEmpty()){
          
      if(servletRequest.getPathInfo().startsWith("/"))
        targetUri = servletRequest.getPathInfo().substring(1);
      else
        targetUri = servletRequest.getPathInfo();
      
      targetUri = targetUri.replace(targetProxy+"/","");
	...
	proxyRequestUri = rewriteUrlFromRequest(servletRequest,currentProxyConfig,targetUri);
      
    //correct proxy uri
    proxyRequestUri = proxyRequestUri.replace(targetProxy+"/","");
    ...
    HttpClient proxyClient    = (HttpClient)currentProxyConfig.get("proxyClient");    
    
    /*if(targetUri.compareTo("*")){
      targetUri = "";
    }*/
        
    
    Boolean    doLog          = (Boolean)currentProxyConfig.get("doLog"); 
    ...
    setXForwardedForHeader(servletRequest, proxyRequest,currentProxyConfig);
  ...
  private void setXForwardedForHeader(HttpServletRequest servletRequest,
    ...
    if (/*doForwardIP*/ config.get("doForwardIP") != null && ((Boolean)config.get("doForwardIP"))) {
   ...
   */
  protected String rewriteUrlFromRequest(HttpServletRequest servletRequest,Map config,String targetUri) {
    StringBuilder uri = new StringBuilder(500);
    uri.append(targetUri);
    // Handle the path given to the servlet
    if (servletRequest.getPathInfo() != null) {//ex: /my/path.html
      if(!config.get("targetUri").toString().isEmpty())
    ...
    if (queryString != null && queryString.length() > 0) {
      ...
      if ( (config == null || ((Boolean)config.get("doSendUrlFragment"))) && fragIdx >= 0) {
        uri.append('#');
